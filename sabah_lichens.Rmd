---
title: 'How much taxonomic expertise is necessary for prioritising areas of lowland
  rainforest sites as habitats for lichens in northeast Borneo?'
author: 'Rob Mrowicki'
...
The variables identified by the BIOENV analysis were then used in a canonical analysis of principal coordinates (CAP; Anderson & Willis, 2003, _Ecology_ 84:511--525) to enable dissimilarities in lichen communities to be visualised in relation to tree functional traits. The resulting ordination plot incorporated vectors representing Spearman rank correlations between environmental variables and the first two CAP axes. The significance of the overall model (based on the sum of all eigenvalues) and of constraining variables (marginal terms) were assessed via ANOVA-like permutation tests involving `r format(n_perm, big.mark = ",")` permutations. Spearman rank correlation coefficients between individual taxonomic or functional group abundances and CAP axes were calculated to identify the most important groups (i.e. $|\rho| \geq 0.35$) contributing to variability in lichen assemblage structure.
...
# ~ taxonomic groups:
# ~~ richness:
kable(li_taxa_s, caption = 'Lichen taxonomic group _S_ vs. site')
kable(li_taxa_s_ph, caption = 'Post-hoc pairwise tests')
# ~~ diversity:
kable(li_taxa_h, caption = 'Lichen taxonomic group _H\'_ vs. site')
kable(li_taxa_h_ph, caption = 'Post-hoc pairwise tests')

# ~ functional groups:
# ~~ richness:
kable(li_func_s, caption = 'Lichen functional group _S_ vs. site')
kable(li_func_s_ph, caption = 'Post-hoc pairwise tests')
# ~~ diversity:
kable(li_func_h, caption = 'Lichen functional group _H\'_ vs. site')
kable(li_func_h_ph, caption = 'Post-hoc pairwise tests')
```
...
# ~ taxonomic groups:
kable(
  rownames_to_column(simp_li_taxa[[1]], "Tax. group"),
...
# ~ functional groups:
kable(
  rownames_to_column(simp_li_func[[1]], "Func. group"),
...
CAP axis 1 correlated positively with tree girth, while axis 2 correlated negatively with presence of buttresses/flutes; however, only tree girth was significant as a constraining variable (taxonomic groups: pseudo-_F_~1,261~ = `r rep_2dp(cap_taxa_margin_f["girth"])`, P `r rep_p(cap_taxa_margin_p["girth"])`; functional groups: pseudo-_F_~1,261~ = `r rep_2dp(cap_func_margin_f["girth"])`, P `r rep_p(cap_func_margin_p["girth"])`), suggesting that it was more important than buttress/flute presence in terms of structuring lichen communities.

![(a) CAP ordination plot of lichen community structure (taxonomic groups) according to environmental variables (vectors represent Spearman rank correlation coefficients with axes). (b) Spearman rank correlation coefficients between CAP axes and individual taxa for which $|\rho| \geq 0.35$.](./figs/cap_taxa.pdf)

![(a) CAP ordination plot of lichen community structure (functional groups) according to environmental variables (vectors represent Spearman rank correlation coefficients with axes). (b) Spearman rank correlation coefficients between CAP axes and individual functional groups for which $|\rho| \geq 0.35$.](./figs/cap_func.pdf)

Taxonomic groups (R-Es, R-Py and ArH) and functional groups (crapo, crbya, crbysp, sqhya and sqhyp) appeared to be associated with the presence of buttresses/flutes and smaller girth; (PoR and Gr-th) and (crpen and crthe) with smaller girth; and (ArC) and (crstd) with the absence of buttresses/flutes.
...
1. Again, coding 'bark type' numerically (i.e. an ordinal categorical variable) -- so that it can be used as an environmental variable in the CAP analysis -- is questionable; as Holger suggests, water holding capacity is probably more relevant than roughness if we are to do this. (As it happens, bark type is not identified as an 'important' environmental variable anyway, at least not when treated this way; results may differ if it is included as a non-ordinal categorical variable.)
